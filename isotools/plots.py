...
def plot_diff_results(
    result_table, min_support=3, min_diff=0.1, grid_shape=(5, 5), splice_types=None
):
    ...
    groups = {
        gn: [c[:-10] for c in result_table.columns if c.endswith(gn + "_total_cov")]
    ...
    for idx, row in result_table.iterrows():
        ...
        psi_gr = {
            gn: [
                row[f"{sa}_in_cov"] / row[f"{sa}_total_cov"]
                for sa in gr
                if row[f"{sa}_total_cov"] > 0
            ]
            for gn, gr in groups.items()
        }
        support = {
            s: sum(
                abs(i - params_alt[s][0]) < abs(i - params_alt[o][0]) for i in psi_gr[s]
            )
            for s, o in zip(group_names, reversed(group_names))
        }
        ...
        if (
            ...
            logger.debug(
                f'{row.gene} with {"vs".join(str(p[0]) for p in params_alt.values())}'
            )
            continue
        ...
        sns.swarmplot(
            data=pd.DataFrame(list(psi_gr.values()), index=psi_gr).T, ax=ax, orient="h"
        )
...
def plot_embedding(
    ...
    **kwargs,
    ...
    if groups is None:
        ...
        sa_group = {
            sa: gn for gn, salist in groups.items() for sa in salist if sa in samples
        }
...
def plot_distr(
    counts, ax=None, density=False, smooth=None, legend=True, fill=True, **axparams
):
    ...
    for gn, gc in counts.items():
        ...
        if fill:
            ax.fill_between(x, 0, gc / sz, alpha=0.5)
