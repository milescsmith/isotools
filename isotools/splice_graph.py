...
class SegmentGraph:
    ...
    def __init__(self, transcripts, strand):
        ...
        self._tss = [
            start_idx[e[0][0]] for e in transcripts
        ]  # todo: this is missleading: on - strand this would not be the tss
    ...
    def search_transcript(self, exons):
        ...
        if len(exons) == 1:
            return [
                trid
                for trid, (j1, j2) in enumerate(zip(self._tss, self._pas))
                if self._is_same_exon(trid, j1, j2)
        ...
        for i, e in enumerate(exons[:-1]):
            while (
                ...
                tr -= set(
                    trid
                    for trid, j2 in self[j].suc.items()
    ...
    def _is_same_exon(self, tr_nr, j1, j2):
        ...
        for j in range(j1, j2):
            if (
                tr_nr not in self[j].suc
                or self[j].suc[tr_nr] > j + 1
    ...
    def _count_introns(self, tr_nr, j1, j2):
        ...
        logger.debug(
            "counting introns of transcript %i between nodes %i and %i", tr_nr, j1, j2
        )
        delta = 0
        if j1 == j2:
            return 0
    ...
    def get_node_matrix(self) -> np.array:
        ...
        return np.array(
            [
                [
                    True if tss == j or trid in n.pre else False
                    for j, n in enumerate(self)
                ]
                for trid, tss in enumerate(self._tss)
            ]
        )
    ...
    def find_fragments(self):
        ...
        for trid, (tss, pas) in enumerate(zip(self._tss, self._pas)):
            ...
            contains[trid] = {
                trid2
                for trid2, (tss2, pas2) in enumerate(zip(self._tss, self._pas))
                if trid2 != trid
                and tss2 >= tss
                and pas2 <= pas
                and all(nodes[trid2, tss2 : pas2 + 1] == nodes[trid, tss2 : pas2 + 1])
            }
        ...
        for big, smallL in contains.items():
            if big not in truncated:
                for trid in smallL:
                    ...
                    fragments.setdefault(trid, []).append(
                        (big, delta1, delta2)
                        if self.strand == "+"
    ...
    def get_alternative_splicing(self, exons, alternative=None):
        ...
        if (
            alternative is not None and len(alternative) > 0
        ...
        if (
            len(exons) > 1
            and not any(j in self._tss for j in range(j1, j2 + 1))  # no mono exon
            and self[j1].start <= exons[0][0]  # no tss/pas within exon
        ):  # start of first exon is exonic in ref
            j0 = max(
                self._tss[trid] for trid in self[j1].pre
        ...
        if (
            len(exons) > 1
            and j2 >= j1
            and not any(j in self._pas for j in range(j1, j2 + 1))
            and self[j2].end >= exons[-1][1]  # no tss/pas within exon
        ):  # end of last exon is exonic in ref
            try:
                j3 = min(
                    self._pas[trid] for trid in self[j2].suc
    ...
    def _check_exon(self, j1, j2, is_first, is_reverse, e, e2=None):
        ...
        if (
            j1 > j2
        ):  # e is not contained at all  -> novel exon (or TSS/PAS if first/last)
            category = 3
            if is_first or is_last:
                altsplice = {
                    "novel intronic PAS"
                    if is_first == is_reverse
                    else "novel intronic TSS": [e]
                }
            else:
            ...
            if self[j2][1] != e[1]:  # second splice site missmatch
                if not is_last:
                    ...
                    dist = min(
                        (self[j][1] - e[1] for j in range(j1, j2 + 1)),
                        key=lambda x: abs(x),
                    )  # the distance to next junction
                    ...
                    category = 3
                elif self[j2][1] < e[1] and not any(
                    j in self._pas for j in range(j1, j2 + 1)
                ):  # exon end is intronic in ref & not overlapping tss
        ...
        if j1 < j2 and any(
            ...
            if (
                ...
                if troi:
                    ...
                    while j < j2:
                        nextj = min(
                            js for trid, js in self[j].suc.items() if trid in troi
                        )
    ...
    def fuzzy_junction(self, exons, size):
        ...
        for i, e1 in enumerate(exons[:-1]):
            ...
            try:
                j1 = next(
                    j for j in range(j1, len(self)) if self[j].end + size >= e1[1]
                )
            ...
            shift = None
            while (
                j1 < len(self) and self[j1].end - e1[1] <= size
    ...
    def get_intron_support_matrix(self, exons):
        ...
        node_iter = iter(self)
        ism = np.zeros(
            (len(self._tss), len(exons) - 1), np.bool
    ...
    def get_exon_support_matrix(self, exons):
        ...
        esm = np.zeros(
            (len(self._tss), len(exons)), np.bool
        )  # the intron support matrix
    ...
    def _splice_dependence(self, sidx, min_cov, coverage, ignore_unspliced=True):
        ...
        for j in jumps:
            ...
            if ignore_unspliced:
                nojump_ids = {
                    trid
                    for trid in jstart.intersection(jend)
                    if self._is_spliced(trid, j[0], j[1])
                }
            else:
        ...
        for j1 in nonjumps:
            for j2 in (j for j in nonjumps if j[0] > j1[1]):
                ...
                yield ma, (self[j1[0]].end, self[j1[1]].start), (
                    self[j2[0]].end,
                    self[j2[1]].start,
                )
    ...
    def _is_spliced(self, trid, ni1, ni2):
        ...
        if any(
            self[i].end < self[i + 1].start for i in range(ni1, ni2)
    ...
    def find_start_end_events(self):
        ...
        for (
            ...
            alt_tr = [
                tr for tr, pas in enumerate(self._pas) if tr not in tr_set and pas > n
            ]
            yield (
                alt_tr,
                list(tr_set),
                tss_start[n],
                n,
                "TSS" if self.strand == "+" else "PAS",
            )
        for n, tr_set in pas.items():
            alt_tr = [
                tr for tr, tss in enumerate(self._tss) if tr not in tr_set and tss < n
            ]
            yield (
                alt_tr,
                list(tr_set),
    ...
    def from_transcript_list(cls, transcripts, strand) -> "SpliceGraph":
        ...
        for tr_nr, tr in enumerate(transcripts):
            ...
            for i, (b1, b2) in enumerate(pairwise(pos for e in tr for pos in e)):
                ...
                sg._graph[b2, bool(i % 2)][1][tr_nr] = b1, not bool(
                    (i) % 2
                )  # successor
                sg._graph[b1, not bool(i % 2)][0][tr_nr] = b2, bool(
                    (1) % 2
                )  # predesessor
        return sg
