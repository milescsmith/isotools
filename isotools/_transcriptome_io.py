...
def add_short_read_coverage(self, bam_files, names=None, load=False):
    ...
    assert isinstance(
        bam_files, dict
    ), "either provide a bam file and names as strings, lists of strings or as a dict"
    names = list(bam_files)
    bam_files = list(bam_files.values())
    assert all(
        ~self.infos["short_reads"].name.isin(names)
    ), "Trying to add short read track that is present already."

    self.infos["short_reads"] = pd.concat(
        [self.infos["short_reads"], pd.DataFrame({"name": names, "file": bam_files})],
        ignore_index=True,
    )
    if (
        load
    ...
    for g in self:
        ...
        for i, tr in enumerate(g.transcripts):
            if any(s in tr["coverage"] for s in sample_names):
                tr["coverage"] = {
                    s: cov for s, cov in tr["coverage"].items() if s not in sample_names
                }
                if not tr["coverage"]:
                    remove_tr.append(i)
        if (
            ...
            g.data["transcripts"] = [
                tr for i, tr in enumerate(g.transcripts) if i not in remove_tr
            ]
...
def add_sample_from_bam(
    ...
    **kwargs,
            ...
            for (
                ...
                for read in align.fetch(chrom):
                    ...
                    if tr_range[0] < 0 or tr_range[1] > chr_len:
                        logger.error(
                            f"Alignment outside chromosome range: transcript at {tr_range} for chromosome {chrom} of length {chr_len}"
                        )
                        continue
                    if "is" in tags:
                        cov = tags[
                    ...
                    if (
                        "SA" in tags or read.flag & 0x800
                    ):  # part of a chimeric alignment
                        if chimeric_mincov > 0:  # otherwise ignore chimeric read
                            chimeric.setdefault(read.query_name, [cov, []])
                            assert (
                                chimeric[read.query_name][0] == cov
                            ), "error in bam: parts of chimeric alignment for read {} has different coverage information {} != {}".format(
                    ...
                    for tr_interval in transcripts.overlap(
                        *tr_range
                    ...
                    if 4 in read.cigartuples:  # clipping
                        clip = get_clipping(
                            read.cigartuples, read, read.reference_start
                        )
                        tr.setdefault("clipping", {}).setdefault(
                            sample_name, {}
                        ).setdefault(clip, 0)
                ...
                for tr_interval in transcripts:
                    ...
                    gene = self._add_sample_transcript(
                        tr, chrom, sample_name, fuzzy_junction
                    )
                    if gene is None:
    ...
    if n_secondary > 0:
        logger.info(
            f"skipped {n_secondary} secondary alignments (0x100), alignment that failed quality check (0x200) or PCR duplicates (0x400)"
        )
    if unmapped > 0:
    ...
    if len(non_chimeric):
        logger.info(
            f"imported {len(non_chimeric)} chimeric alignments that can be chained to single nonchimeric transcripts (long intron alingment split)"
        )
    ...
    if len(chimeric) - n_chimeric > 0:
        logger.info(
            f"ignoring {len(chimeric)-n_chimeric} chimeric alignments with less than {chimeric_mincov} reads"
        )
    total_nc_reads += sum(nc[0] for nc in non_chimeric)  # this adds long introns
    chimeric_msg = (
        ""
        if not n_chimeric
        else f" and {n_chimeric} chimeric reads with coverage of at least {chimeric_mincov}"
    )
    logger.info(
        f"imported {total_nc_reads} nonchimeric reads{chained_msg}{chimeric_msg}."
    )
    novel = dict()
    for (cov, (chrom, strand, exons, _, _), introns) in non_chimeric:
        try:
            ...
            tr = {
                ...
                "strand": strand,
                "chr": chrom,
                "long_intron_chimeric": {sample_name: {introns: cov}},
            }
        except:
            logger.error(
    ...
    for chrom in novel:
        self._add_novel_genes(
            IntervalTree(
                Interval(tr["exons"][0][0], tr["exons"][-1][1], tr)
                for tr in novel[chrom]
            ),
            chrom,
            sample_name,
        )
    ...
    for g in self:
        if (
            "coverage" in g.data and g.data["coverage"] is not None
            ...
            for found in self.chimeric.setdefault(new_bp, []):
                ...
                for part in new_chim[1]:
                    if part[0] in self.data:
                        genes_ol = [
                            g
                            for g in self.data[part[0]][part[2][0][0] : part[2][-1][1]]
                            if g.strand == part[1]
                        ]
                        # g,_=_get_intersects(genes_ol, part[2])
                        g, _, _, _ = _find_splice_sites(
                            genes_ol, part[2]
                        )  # take the best - ignore other hits here
                        if g is not None:
                            part[4] = g.name
                            g.data.setdefault("chimeric", {})[new_bp] = self.chimeric[
                                new_bp
                            ]
    return total
...
def _check_chimeric(chimeric):
    ...
    for new_chim in chimeric.values():
        ...
        merge = [
            ...
            if bp[0] == bp[3]
            and bp[1] == bp[4]  # same chr
            and 0  # same strand,
            < (bp[5] - bp[2] if bp[1] == "+" else bp[2] - bp[5])
            < 1e6
        ...
        if bpts:
            ...
            non_chimeric.append(
                [new_chim[0], new_chim[1][0], tuple(merged_introns)]
            )  # coverage and "long introns"
    if skip:
        logger.warning(
            f"ignored {skip} chimeric alignments with only one part aligned to specified chromosomes."
        )
...
def _add_sample_transcript(self, tr, chrom, sample_name, fuzzy_junction=5):
    ...
    g, ref_ol, additional, not_covered = _find_splice_sites(
        genes_ol_strand, tr["exons"]
    )
    if g is not None:
        if (
            g.is_annotated
        ):  # check for fuzzy junctions (e.g. same small shift at 5' and 3' compared to reference annotation)
            shifts = g.correct_fuzzy_junctions(
                tr, fuzzy_junction, modify=True
            )  # this modifies tr['exons']
            if shifts:
                tr.setdefault("fuzzy_junction", {}).setdefault(sample_name, []).append(
                    shifts
                )  # keep the info, mainly for testing/statistics
                for (
                    tr2
                ) in g.transcripts:  # check if correction made it identical to existing
                    if splice_identical(tr2["exons"], tr["exons"]):
                        tr2.setdefault("fuzzy_junction", {}).setdefault(
                            sample_name, []
                        ).append(
                            shifts
                        )  # keep the info, mainly for testing/statistics
                        _combine_transcripts(tr2, tr, sample_name)
                        return g
            tr["annotation"] = g.ref_segment_graph.get_alternative_splicing(
                tr["exons"], additional
            )
            if not_covered:
                tr[
...
def _combine_transcripts(established, new_tr, sample_name):
    ...
    try:
        established["coverage"][sample_name] = (
            established["coverage"].get(sample_name, 0) + new_tr["coverage"]
        )
        ...
        for side in "TSS", "PAS":
            for pos, cov in new_tr[side].items():
                established[side].setdefault(sample_name, {})[pos] = (
                    established[side].get(sample_name, {}).get(pos, 0) + cov
                )
        # find median tss and pas
        starts = [
            v for sa in established["TSS"] for v in established["TSS"][sa].items()
        ]
        ...
        if "long_intron_chimeric" in new_tr:
            for introns in new_tr["long_intron_chimeric"][sample_name]:
                established.setdefault("long_intron_chimeric", {}).setdefault(
                    sample_name, {}
                ).setdefault(introns, 0)
                established["long_intron_chimeric"][sample_name][introns] += new_tr[
                    "long_intron_chimeric"
                ][sample_name][introns]
    except:
...
def _add_novel_genes(
    self, novel, chrom, sa, spj_iou_th=0, reg_iou_th=0.5, gene_prefix="PB_novel_"
):
    ...
    for i, tr in enumerate(novel):
        ...
        candidates = [
            c
            for c in novel.overlap(tr.begin, tr.end)
            if c.data["strand"] == tr.data["strand"] and idx[id(c)] < i
        ]
        for c in candidates:
            if c in merge[i]:
                continue
...
def _find_splice_sites(genes_ol, exons):
    ...
    if genes_ol:
        ref_ol = {
            g.name: g.ref_segment_graph.get_overlap(exons)
            for g in genes_ol
            if g.is_annotated
        }
        if len(exons) == 1:  # no splice sites, but return gene with largest overlap
            ol = np.array(
                [
                    ref_ol[g.name]
                    if g.is_annotated
                    else g.segment_graph.get_overlap(exons)
                    for g in genes_ol
                ]
            )
            best_idx = ol.argmax()
            ...
            splice_sites = np.array(
                [
                    g.ref_segment_graph.find_splice_sites(exons)
                    if g.is_annotated
                    else g.segment_graph.find_splice_sites(exons)
                    for g in genes_ol
                ]
            )
            ...
            try:  # find index of reference gene that covers the most splice sites
                best_idx = next(
                    idx
                    for idx in np.argsort(-sum_ol)
                    if genes_ol[idx].is_annotated and sum_ol[idx] > 0
                )
            ...
            if sum_ol[best_idx] > 0:
                ...
                elsefound = [
                    (g.name, not_in_best[a])
                    for g, a in zip(genes_ol, additional)
                    if a.sum() > 0
                ]  # genes that cover additional splice sites
                notfound = (
                    splice_sites.sum(0) == 0
                ).nonzero()  # not covered splice sites
...
def _get_intersects(genes_ol, exons):
    ...
    intersect = [
        (i, g.ref_segment_graph.get_intersects(exons))
        for i, g in enumerate(genes_ol)
        if g.is_annotated
    ]
    if intersect:
        best_idx, intersects = max(intersect, key=lambda x: x[1])
        if intersects[0] > 0 or (
            len(exons) == 1 and intersects[1] > 0
        ):  # at least one splice overlap for multiexon genes
            return genes_ol[best_idx], intersects
    # no annotated overlap, look for novel genes
    intersect = [
        (i, g.segment_graph.get_intersects(exons))
        for i, g in enumerate(genes_ol)
...
def import_gtf_transcripts(fn, transcriptome, chromosomes=None):
    ...
    for line in tqdm(gtf.fetch(), smoothing=0.1):
        ...
        info = dict(
            [
                pair.lstrip().split(" ", 1)
                for pair in ls[8].replace('"', "").split(";")
                if pair
            ]
        )
        ...
        if ls[2] == "exon":
            ...
            try:
                ...
                logger.warn(
                    "gtf format error: exon without transcript_id. Skipping line\n"
                    + line
                )
        elif ls[2] == "gene":
            ...
            try:
                _ = transcripts.setdefault(info["gene_id"], list()).append(
                    info["transcript_id"]
                )
            except KeyError:
                logger.warn(
    ...
    if missed_genes:
        raise ValueError(
            "no specific gene information for {}/{} genes".format(
                missed_genes, missed_genes + sum((len(t) for t in genes.values()))
            )
        )
    ...
    for chrom in genes:  # link transcripts to genes
        for gene in genes[chrom]:
            ...
            for t_id in t_ids:
                try:
                    gene.data["reference"]["transcripts"] = [
                        {"transcript_id": t_id, "exons": exons[t_id]}
                    ]
                except KeyError:
                    # genes without transcripts get a single exons transcript
                    gene.data["reference"]["transcripts"] = [
                        {"transcript_id": t_id, "exons": [tuple(gene[:2])]}
                    ]
    return genes
            ...
            for itype in immune:
                ...
                for i, g in enumerate(immune[itype]):
                    ...
                    if (
                        ...
                        or g.end - immune[itype][i + 1].start > maxgap
                    ):
                        ref_info = {
                            "gene_type": f"{itype}_gene",
                            "transcripts": [
                                t
                                for g in immune[itype][offset : i + 1]
                                for t in g.ref_transcripts
                            ],
                        }
                        info = {
                        ...
                        offset = i + 1
    logger.info(
        f'collapsed {num["IG"]} immunoglobulin loci and  {num["TR"]} T-cell receptor loci'
    )


def import_gff_transcripts(
    fn, transcriptome, chromosomes=None, gene_categories=["gene"], short_exon_th=25
):
    ...
    for line in tqdm(gff.fetch(), smoothing=0.1):
        ...
        try:
            ...
            logger.warning(
                "GFF format error in infos (should be ; seperated key=value pairs). Skipping line:\n"
                + line
            )
        start, end = [int(i) for i in ls[3:5]]
        start -= 1  # to make 0 based
        if ls[2] == "exon":
            try:
                ...
                logger.warning(
                    "GFF format error: no parent found for exon. Skipping line: " + line
                )
    ...
    if missed_genes:
        ...
        logger.warning(
            "Missing genes! Found gene information in categories {} for {}/{} genes".format(
                gene_categories, found, found + notfound
            )
        )
        ...
        for gene in genes[chrom]:
            ...
            for t_id, tr_info in tr.items():
                ...
                if t_id in cds_start and t_id in cds_stop:
                    tr_info["CDS"] = (
                        (cds_start[t_id], cds_stop[t_id])
                        if cds_start[t_id] < cds_stop[t_id]
                        else (cds_stop[t_id], cds_start[t_id])
                    )
                gene.data["reference"].setdefault("transcripts", []).append(tr_info)
            if short_exon_th is not None:
                short_exons = {
                    e
                    for tr in gene.data["reference"]["transcripts"]
                    for e in tr["exons"]
                    if e[1] - e[0] <= short_exon_th
                }
                if short_exons:
        ...
        for pos, v in mut.items():
            for alt in v.values():
                alt[1] = (
                    ""
                    if alt[1] < 0
                    else genome_fh.fetch(read.reference_name, pos, pos + alt[1])
                )
            for tr in exons:
                for e in tr:
    ...
    for pref, alt in alias.items():
        ...
        if pref not in d:
            try:
                ...
                logger.error(
                    f"did not find alternative for {pref}- suggested terms are {alt}, but have only those keys:{list(d)}"
                )
                raise
        for a in alt:
            d.pop(a, None)
...
def gene_table(self, region=None):  # ideas: filter, extra_columns
    ...
    rows = [
        (g.chrom, g.start, g.end, g.strand, g.id, g.n_transcripts)
...
def transcript_table(
    ...
    extracolnames = {
        "annotation": ("novelty_class", "novelty_subclasses"),
        "coverage": (f"coverage_{sn}" for sn in self.samples),
    }
    ...
    colnames = [
        ...
        n
        for w in extra_columns
        for n in (extracolnames[w] if w in extracolnames else (w,))
    ]
    rows = []
    for g, trid, tr in self.iter_transcripts(
        region, include, remove, min_coverage, max_coverage
    ):
        rows.append(
            [
                g.chrom,
            ...
            + g.get_infos(trid, extra_columns)
        )
    df = pd.DataFrame(rows, columns=colnames)
    return df
...
def chimeric_table(
    self, region=None, include=None, remove=None
...
def write_gtf(
    self, fn, source="isotools", use_gene_name=False, include=None, remove=None
):
        ...
        for g in self.iter_genes(region, include, remove):
            ...
            for setA, setB, nodeX, nodeY, splice_type in seg_graph.find_splice_bubbles(
                ...
                lines = alt_splice_export(
                    setA, setB, nodeX, nodeY, st, seg_graph, g, count[st]
                )
                if lines:
                    count[st] += len(lines)
                    fh[st].write(
...
def _miso_alt_splice_export(setA, setB, nodeX, nodeY, st, seg_graph, g, offset):
    ...
    lines.append(
        [
            ...
            g.strand,
            ".",
            f"ID={event_id};gene_name={g.name};gene_id={g.id}",
        ]
    )
    ...
    for i, exons in enumerate(
        {tuple(seg_graph._get_all_exons(nodeX, nodeY, tr)) for tr in setA}
    ):
        lines.append(
            (
                g.chrom,
    ...
    for i, exons in enumerate(
        {tuple(seg_graph._get_all_exons(nodeX, nodeY, tr)) for tr in setB}
    ):
        lines.append(
            (
                g.chrom,
...
def _mats_alt_splice_export(setA, setB, nodeX, nodeY, st, seg_graph, g, offset):
    ...
    for exonsB in {
        ...
        if st in ["A3SS", "A5SS"] and len(exonsB) == 2:
            ...
            assert (
                exonsA[0] == exonsB[0] and exonsA[1] == exonsB[2]
            ), f"invalid exon skipping {exonsA} vs {exonsB}"  # just to be sure everything is consistent
            ...
            for exonsA in {
                ...
                assert (
                    len(exonsA) == 3
                    and exonsA[0] == exonsB[0]
                    and exonsA[2] == exonsB[2]
        ...
        if exons_sel is not None:
            lines.append(
                [f'"{g.id}"', f'"{g.name}"', chrom, g.strand]
                + [pos for e in exons_sel for pos in e]
            )
...
class IntervalArray:
    ...
    def overlap(self, begin, end):
        try:
            ...
            logger.error(
                f"requesting interval between {begin} and {end}, but array is allocated only until position {len(self.data)*self.bin_size}"
            )
            raise
        return (
            self.obj[obj_id]
            for obj_id in candidates
            if overlap((begin, end), self.obj[obj_id])
    ...
    def add(self, obj):
        ...
        try:
            ...
            logger.error(
                f"adding interval from {obj.begin} to {obj.end}, but array is allocated only until position {len(self.data)*self.bin_size}"
            )
            raise
