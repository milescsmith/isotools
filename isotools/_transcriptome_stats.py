...
def betabinom_lr_test(x, n):
    ...
    try:
        ...
        l1 = (
            betabinom_ll(x[0], n[0], *params[0]).sum()
            + betabinom_ll(x[1], n[1], *params[1]).sum()
        )
        ...
        raise
    return chi2.sf(2 * (l1 - l0), 2), [
        v for p in params_alt for v in p
...
def altsplice_test(
    ...
    for g in tqdm(self):
        ...
        if g.is_annotated and g.n_transcripts:
            ...
            for _, _, nX, nY, splice_type in sg.find_splice_bubbles(
                start_end_events=True
            ):
    ...
    try:
        ...
        logger.error(
            "unexpected error during calculation of adjusted p-values: {e}", e=e
        )
    return df


def splice_dependence_test(
    self, samples=None, min_cov=20, padj_method="fdr_bh", region=None
):
    logger.warning(
        "splice_dependence_test is depreciated - revise based on splice bubbles"
    )
    ...
    for g in tqdm(self.iter_genes(region=region)):
        for ma, j1, j2 in g.segment_graph.splice_dependence(sa_idx, min_cov):
        ...
        for setA, setB, nX, nY, splice_type in sg.find_splice_bubbles(
            ...
            if (
                ...
                if splice_type in ["TSS", "PAS"]:
                    ...
                    novel = (start, end) not in known.get(splice_type, set())
                bubbles.append(
                    [g.id, g.chrom, start, end, splice_type, novel]
                    + list(junction_cov)
                    + list(total_cov)
                )
    return pd.DataFrame(
...
def altsplice_stats(
    self, groups=None, weight_by_coverage=True, min_coverage=2, tr_filter={}
):
    ...
    for g, trid, tr in self.iter_transcripts(**tr_filter):
        if g != current:
            ...
            w = (
                g.coverage.copy()
                if groups is None
                else np.array([g.coverage[grp, :].sum(0) for grp in groups.values()])
            )
            w[w < min_coverage] = 0
        ...
        if "annotation" not in tr or tr["annotation"] is None:
            weights["unknown"] = (
                weights.get("unknown", np.zeros(w.shape[0])) + w[:, trid]
            )
        else:
...
def transcript_length_hist(
    ...
    for g, trid, tr in self.iter_transcripts(**tr_filter):
        ...
        trlen.append(
            sum(e[1] - e[0] for e in tr["exons"]) if use_alignment else tr["source_len"]
    ...
    if add_reference:
        ref_len = [
            sum(e[1] - e[0] for e in tr["exons"])
            for _, _, tr in self.iter_ref_transcripts(**ref_filter)
        ]
...
def exons_per_transcript_hist(
    ...
    if add_reference:
        ref_n_exons = [
            len(tr["exons"]) for _, _, tr in self.iter_ref_transcripts(**ref_filter)
        ]
...
def downstream_a_hist(
    ...
    counts = pd.DataFrame(
        {
            gn: np.histogram(acontent, weights=g_cov, bins=bins)[0]
            for gn, g_cov in cov.items()
        }
    )
    if add_reference:
        ref_acontent = [
            tr["downstream_A_content"]
            for _, _, tr in self.iter_ref_transcripts(**ref_filter)
...
def direct_repeat_hist(
    ...
    for g, trid, tr in self.iter_transcripts():
        ...
        if (
            g.is_annotated
            and "annotation" in tr
            and "novel 5' splice site" in tr["annotation"][1]
            and "novel 3' splice site" in tr["annotation"][1]
        ):
            sg = g.ref_segment_graph  # check exonic overlap in the reference annotation
            novel1 = [
                alt
                for alt in tr["annotation"][1]["novel 5' splice site"]
                if sg.is_exonic(alt[0])
            ]
            novel2 = [
                alt
                for alt in tr["annotation"][1]["novel 3' splice site"]
                if sg.is_exonic(alt[0])
            ]
            if g.strand == "-":
            ...
            e1 = [
                next(i for i, e in enumerate(tr["exons"]) if e[1] == alt[0])
                for alt in novel1
            ]  # alt[0] is the genomic position
            e2 = [
                next(i for i, e in enumerate(tr["exons"]) if e[0] == alt[0])
                for alt in novel2
            ]
            candidates = [e for e in e1 if e + 1 in e2]
            nc = (
                {v[0] for v in tr["noncanonical_splicing"]}
                if "noncanonical_splicing" in tr
                else {}
            )
            ...
            novel_rl.extend(
                (tr["direct_repeat_len"][c], g.coverage[:, trid])
                for c in candidates
                if c in nc
            )
    ...
    if groups is not None:
        known_rl_cov = pd.DataFrame(
            {grn: known_rl_cov[grp].sum(1) for grn, grp in groups.items()}
        )
        novel_rl_cov = pd.DataFrame(
            {grn: novel_rl_cov[grp].sum(1) for grn, grp in groups.items()}
        )
    ...
    counts = pd.DataFrame(
        {
            f"{sa} {k}": np.histogram(
                [val[0] for val in v], weights=cov[k][sa], bins=bins
            )[0]
            for k, v in rl.items()
            for sa in (self.samples if groups is None else groups)
        }
    )
