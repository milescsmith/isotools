...
class Coverage:
    ...
    def load(self):
        ...
        with AlignmentFile(self.bam_fn, "rb") as align:
            logger.debug(
                f"Illumina coverage of region {self.reg[0]}:{self.reg[1]}-{self.reg[2]} is loaded from {self.bam_fn}"
            )  # info or debug?
    ...
    def __getitem__(self, subscript):
        if isinstance(subscript, slice):
            return self.profile[
                slice(
                    None if subscript.start is None else subscript.start - self.reg[1],
                    None if subscript.stop is None else subscript.stop - self.reg[1],
                    subscript.step,
                )
            ]  # does not get extended if outside range
