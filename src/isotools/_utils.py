...
def basequal_hist(
    bam_fn, qual_bins=10 ** (np.linspace(-7, 0, 30)), len_bins=None, n=1000
):
        ...
        with tqdm(total=n, unit=" reads") as pbar:
            for i, read in enumerate(align):
                ...
                if len_bins is not None:
                    len_i = next(
                        (i for i, th in enumerate(len_bins) if l < th), len(len_bins)
                    )
                error_rate = (
                    sum([10 ** (-q / 10) for q in read.query_qualities]) / l * 100
                )
                q_i = next(
