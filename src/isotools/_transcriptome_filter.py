...
def iter_genes(self, region=None, include=None, remove=None):
    ...
    if include or remove:
        assert (
            "filter" in self.infos
        ), "no filter flags found - run .add_filter() method first"
        assert not include or all(
            f in self.infos["filter"]["gene_filter"] for f in include
        ), "not all filters to include found"
        assert not remove or all(
            f in self.infos["filter"]["gene_filter"] for f in remove
        ), "not all filters to remove found"
    if region is None:
        genes = self
        ...
        if region in self.data:
            ...
            try:
                ...
                raise ValueError(
                    'incorrect region {} - specify as string "chr:start-end" or tuple ("chr",start,end)'.format(
                        region
                    )
                )
            if chrom in self.data:
...
def iter_transcripts(
    self, region=None, include=None, remove=None, min_coverage=None, max_coverage=None
):
    ...
    if include or remove:
        ...
        valid_filters = (
            valid_filters
            + list(all_filter["transcript_filter"])
            + list(all_filter["gene_filter"])
        )
        msg = (
            "did not find the following filter flags for {}: {}\nvalid filters are: {}"
        )
        assert not include or all(f in valid_filters for f in include), msg.format(
            "inclusion",
            ", ".join(f for f in include if f not in valid_filters),
            ", ".join(valid_filters),
        )
        assert not remove or all(f in valid_filters for f in remove), msg.format(
            "removal",
            ", ".join(f for f in remove if f not in valid_filters),
            ", ".join(valid_filters),
        )
    ...
    g_include = (
        [f for f in include if f in all_filter["gene_filter"]] if include else []
    )
    g_remove = [f for f in remove if f in all_filter["gene_filter"]] if remove else []
    t_include = (
        [f for f in include if f in all_filter["transcript_filter"]] if include else []
    )
    t_remove = (
        [f for f in remove if f in all_filter["transcript_filter"]] if remove else []
    )

    for g in self.iter_genes(region, g_include, g_remove):
        for i, tr in g.filter_transcripts(
            t_include, t_remove, anno_include, anno_remove, min_coverage, max_coverage
        ):
            yield g, i, tr
...
def iter_ref_transcripts(self, region=None, include=None, remove=None):
    ...
    g_remove = [f for f in remove if f in all_filter["gene_filter"]] if remove else []
    t_include = (
        [f for f in include if f not in all_filter["gene_filter"]] if include else []
    )
    t_remove = (
        [f for f in remove if f not in all_filter["gene_filter"]] if remove else []
    )
    assert all(
        f in all_filter["ref_transcript_filter"] for f in t_include
    ), "not all reference filters to include found"
    assert all(
        f in all_filter["ref_transcript_filter"] for f in t_remove
    ), "not all  reference filters to remove found"
    for g in self.iter_genes(region, g_include, g_remove):
        if g.is_annotated:
...
def _filter_function(argnames, expression):
    ...
    return eval(
        f'lambda {",".join(arg+"=None" for arg in argnames)}: bool({expression})\n',
        {},
        {},
    )
