...
def sashimi_plot_short_reads(
    ...
    ax.fill_between(
        range(start, end),
        0,
        np.log10(cov, where=cov > 0, out=np.nan * cov),
    ...
    for (x1, x2), w in junctions.items():
        ...
        while any(
            _label_overlap(
                (center, log10(max(y1, y2)) + bow_height), tp, text_width, text_height
            )
        ...
        if jparams[priority]["draw_label"]:
            _ = ax.text(
                center,
                log10(max(y1, y2)) + min(bow_height) + text_height / 3,
                w,
...
def gene_track(
    ...
    for g in ol_genes:
        ...
        if reference:  # select transcripts and sort by start
            transcript_list.extend(
                [
                    (g, tr_nr, tr)
                    for tr_nr, tr in enumerate(g.ref_transcripts)
                    if select_tr is None or tr_nr in select_tr
                ]
            )
        else:
            transcript_list.extend(
                [
                    (g, tr_nr, tr)
                    for tr_nr, tr in enumerate(g.transcripts)
                    if select_tr is None or tr_nr in select_tr
                ]
            )
    ...
    for g, tr_nr, tr in transcript_list:
        ...
        if (
            tr_end < x_range[0] or tr_start > x_range[1]
        ...
        trid += (
            tr["transcript_name"] if "transcript_name" in tr else f"{g.name}_{tr_nr}"
        )

        # find next line that is not blocked
        try:
            i = next(
                idx for idx, last in enumerate(blocked) if last < tr["exons"][0][0]
            )
        ...
        ax.plot((tr_start, tr_end), [i + 0.25] * 2, color=color)
        ...
        for j, (st, end) in enumerate(tr["exons"]):
            if (
                "CDS" in tr and tr["CDS"][0] <= end and tr["CDS"][1] >= st
            ):  # CODING exon
                c_st, c_end = max(st, tr["CDS"][0]), min(
                    tr["CDS"][1], end
