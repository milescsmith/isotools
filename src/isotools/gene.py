...
class Gene(Interval):
    ...
    def short_reads(self, idx):
        ...
        try:
            ...
            srdf = self._transcriptome.infos[
                "short_reads"
    ...
    def add_filter(self, gene_filter, transcript_filter, ref_transcript_filter):
        ...
        self.data["filter"] = [
            name
            for name, fun in gene_filter.items()
            if _eval_filter_fun(fun, name, self.data)
        ]
        for tr in self.transcripts:
            tr["filter"] = [
                name
                for name, fun in transcript_filter.items()
                if _eval_filter_fun(fun, name, tr)
            ]
        for tr in self.ref_transcripts:
            tr["filter"] = [
                name
                for name, fun in ref_transcript_filter.items()
                if _eval_filter_fun(fun, name, tr)
            ]
    ...
    def filter_transcripts(
        ...
        for i, tr in enumerate(self.transcripts):
            ...
            if not include or any(f in tr["filter"] for f in include):
                if not anno_include or any(
                    f in tr["annotation"][1] for f in anno_include
                ):
                    if not remove or all(f not in tr["filter"] for f in remove):
                        if not anno_remove or all(
                            f not in tr["annotation"][1] for f in anno_remove
                        ):
                            yield i, tr
        ...
        for i in trids:
            ...
            lines.append(
                (
                    ...
                    self.strand,
                    ".",
                    "; ".join(f'{k} "{v}"' for k, v in info.items() if k != "exon_id"),
                )
            )
    ...
    def add_noncanonical_splicing(self, genome_fh):
        ...
        for tr in self.transcripts:
            ...
            if self.strand == "+":
                ...
                sj_seq = [
                    str(Seq(ss_seq[d] + ss_seq[a]).reverse_complement()) for d, a in pos
                ]
        ...
        for tr in self.transcripts:
            for intron in (
                ...
                if intron not in score:
                    ...
                    align = [
                        a == b
                        for a, b in zip(intron_seq[intron[0]], intron_seq[intron[1]])
                    ]
                    score[intron] = 0
                    ...
                    for a in reversed(align[:delta]):
                        ...
                        score[intron] += 1
        for tr in self.transcripts:
            tr["direct_repeat_len"] = [
                min(score[intron], delta)
                for intron in (
                    (tr["exons"][i][1], tr["exons"][i + 1][0])
                    for i in range(len(tr["exons"]) - 1)
                )
    ...
    def _get_info(self, trid, key):
        ...
        if key == "filter":
            ...
            subcat_string = ";".join(
                k if v is None else "{}:{}".format(k, v) for k, v in subcat.items()
            )
